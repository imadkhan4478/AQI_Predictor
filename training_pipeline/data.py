...
# day-of-month and month are deliberately excluded. With 3 months of data they
# let the model memorise "late July looks like this" -- dropping them collapsed
# R2 from 0.33 to -0.03, which looked like importance but was memorisation. With
# 5.7 years they measurably hurt (0.628 vs 0.645 at 24h). hour is kept, and also
# encoded cyclically below so 23:00 and 00:00 are adjacent.
...
def add_lag_features(df):
    ...
    Every window is shifted by 1 first, so a row never sees its own AQI in its
    own rolling statistics. Reindexing to a continuous grid matters because
    ~5% of hours are missing: shifting by row position across a gap would
    silently label a value "1 hour ago" when it is really several.

    These hurt at 2k rows (R2 0.645 -> 0.211 at 24h) -- too many features for
    too few samples -- and help at 49k (0.645 -> 0.660 at 24h, 0.414 -> 0.468
...
def add_target(df, horizon_hours=FORECAST_HORIZON_HOURS):
    ...
    misalign the label. Also stores the change from now, which is what the
    model actually learns -- see train_delta below."""
...
def time_based_split(X, y, timestamps, horizon_hours=FORECAST_HORIZON_HOURS, test_fraction=0.2):
    """Split chronologically, then purge training rows whose *label* falls
    inside the test window.

    Splitting chronologically is not enough on its own: a training row at
    T_split - 1h carries a label from T_split + 71h, deep inside the test
    period, so the model sees test-period AQI during training. Measured at the
    72h horizon, that inflated R2 from 0.237 to 0.326 -- about 27% of the
    apparent skill was leakage.
    """
...
def load_training_data(horizon_hours=FORECAST_HORIZON_HOURS, target=TARGET_COLUMN):
    ...
    trained on the change: predictions are then anchored to the latest reading
    (prediction = aqi + w * delta), which tracks the pollution level for free.
    That matters because Lahore's mean AQI has fallen from ~387 (2020) to ~162
    (2026), and tree models cannot extrapolate beyond the levels they trained on.
    `current_aqi` is returned so callers can re-anchor and score persistence.
    """
    df = read_features_df()
